...
TEST_CASE("difference")
{
	...
	REQUIRE(mv11.at<1>() == -6.0);
	REQUIRE(mv11.at<2>() == -6.0);
	...
	REQUIRE(mv13.at<1>() == -36.0);
	REQUIRE(mv13.at<2>() == -36.0);
	...
	REQUIRE(mv15.at<1>() == -74.0);
	REQUIRE(mv15.at<2>() == -76.0);
	REQUIRE(mv15.at<3>() == -89.0);

	mv2<0,1,2,3> mv16 = std::get<1>(lhs) - std::get<6>(rhs);

	REQUIRE(mv16.at<0>() == -109.0);
	REQUIRE(mv16.at<1>() == -108.0);
	REQUIRE(mv16.at<2>() == -120.0);
	...
	REQUIRE(mv26.at<3>() == -114.0);

	mv2<0,1,2> mv30 = std::get<3>(lhs) - std::get<0>(rhs);

	REQUIRE(mv30.at<0>() == 20.0);
	...
	REQUIRE(mv31.at<1>() == 18.0);
	REQUIRE(mv31.at<2>() == 18.0);
	...
	REQUIRE(mv33.at<0>() == -14.0);
	REQUIRE(mv33.at<1>() == -12.0);
	REQUIRE(mv33.at<2>() == -12.0);

	mv2<0,1,2,3> mv34 = std::get<3>(lhs) - std::get<4>(rhs);

	REQUIRE(mv34.at<0>() == -36.0);
	...
	REQUIRE(mv35.at<1>() == -50.0);
	REQUIRE(mv35.at<2>() == -52.0);
	REQUIRE(mv35.at<3>() == -89.0);

	mv2<0,1,2,3> mv36 = std::get<3>(lhs) - std::get<6>(rhs);

	REQUIRE(mv36.at<0>() == -86.0);
	REQUIRE(mv36.at<1>() == -84.0);
	REQUIRE(mv36.at<2>() == -96.0);
	REQUIRE(mv36.at<3>() == -131.0);

	mv2<0,3> mv40 = std::get<4>(lhs) - std::get<0>(rhs);

	REQUIRE(mv40.at<0>() == 44.0);
	...
	REQUIRE(mv42.at<3>() == 34.0);

	mv2<0,1,2,3> mv43 = std::get<4>(lhs) - std::get<3>(rhs);

	REQUIRE(mv43.at<0>() == 10.0);
	...
	REQUIRE(mv44.at<0>() == -12.0);
	REQUIRE(mv44.at<3>() == -8.0);
	...
	REQUIRE(mv51.at<1>() == 56.0);
	REQUIRE(mv51.at<2>() == 58.0);
	...
	REQUIRE(mv52.at<3>() == 54.0);

	mv2<0,1,2,3> mv53 = std::get<5>(lhs) - std::get<3>(rhs);

	REQUIRE(mv53.at<0>() == -37.0);
	REQUIRE(mv53.at<1>() == 26.0);
	REQUIRE(mv53.at<2>() == 28.0);
	...
	REQUIRE(mv54.at<3>() == 12.0);

	mv2<1,2,3> mv55 = std::get<5>(lhs) - std::get<5>(rhs);

	REQUIRE(mv55.at<1>() == -12.0);
	REQUIRE(mv55.at<2>() == -12.0);
	REQUIRE(mv55.at<3>() == -16.0);

	mv2<0,1,2,3> mv56 = std::get<5>(lhs) - std::get<6>(rhs);

	REQUIRE(mv56.at<0>() == -109.0);
	REQUIRE(mv56.at<1>() == -46.0);
	REQUIRE(mv56.at<2>() == -56.0);
	REQUIRE(mv56.at<3>() == -58.0);

	mv2<0,1,2,3> mv60 = std::get<6>(lhs) - std::get<0>(rhs);

	REQUIRE(mv60.at<0>() == 94.0);
	...
	REQUIRE(mv61.at<1>() == 90.0);
	REQUIRE(mv61.at<2>() == 90.0);
	...
	REQUIRE(mv62.at<3>() == 88.0);

	mv2<0,1,2,3> mv63 = std::get<6>(lhs) - std::get<3>(rhs);

	REQUIRE(mv63.at<0>() == 60.0);
	REQUIRE(mv63.at<1>() == 60.0);
	REQUIRE(mv63.at<2>() == 60.0);
	REQUIRE(mv63.at<3>() == 107.0);

	mv2<0,1,2,3> mv64 = std::get<6>(lhs) - std::get<4>(rhs);

	REQUIRE(mv64.at<0>() == 38.0);
	REQUIRE(mv64.at<1>() == 101.0);
	REQUIRE(mv64.at<2>() == 103.0);
	REQUIRE(mv64.at<3>() == 46.0);

	mv2<0,1,2,3> mv65 = std::get<6>(lhs) - std::get<5>(rhs);

	REQUIRE(mv65.at<0>() == 97.0);
	REQUIRE(mv65.at<1>() == 22.0);
	REQUIRE(mv65.at<2>() == 20.0);
	REQUIRE(mv65.at<3>() == 18.0);

	mv2<0,1,2,3> mv66 = std::get<6>(lhs) - std::get<6>(rhs);

	REQUIRE(mv66.at<0>() == -12.0);
	REQUIRE(mv66.at<1>() == -12.0);
	REQUIRE(mv66.at<2>() == -24.0);
	REQUIRE(mv66.at<3>() == -24.0);
}
