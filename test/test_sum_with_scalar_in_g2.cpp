...
TEST_CASE("sum")
{
	...
	REQUIRE(mv10.at<0>() == 1002.0);

	mv2<0,1,2> mv11 = std::get<1>(lhs) + 1000.0;

	REQUIRE(mv11.at<0>() == 1000.0);
	REQUIRE(mv11.at<1>() == 5.0);
	REQUIRE(mv11.at<2>() == 7.0);

	mv2<0,3> mv12 = std::get<2>(lhs) + 1000.0;

	REQUIRE(mv12.at<0>() == 1000.0);
	REQUIRE(mv12.at<3>() == 17.0);

	mv2<0,1,2> mv13 = std::get<3>(lhs) + 1000.0;

	REQUIRE(mv13.at<0>() == 1023.0);
	REQUIRE(mv13.at<1>() == 29.0);
	REQUIRE(mv13.at<2>() == 31.0);

	mv2<0,3> mv14 = std::get<4>(lhs) + 1000.0;

	REQUIRE(mv14.at<0>() == 1047.0);
	REQUIRE(mv14.at<3>() == 53.0);

	mv2<0,1,2,3> mv15 = std::get<5>(lhs) + 1000.0;

	REQUIRE(mv15.at<0>() == 1000.0);
	REQUIRE(mv15.at<1>() == 67.0);
	REQUIRE(mv15.at<2>() == 71.0);
	REQUIRE(mv15.at<3>() == 73.0);

	mv2<0,1,2,3> mv16 = std::get<6>(lhs) + 1000.0;

	REQUIRE(mv16.at<0>() == 1097.0);
	REQUIRE(mv16.at<1>() == 101.0);
	REQUIRE(mv16.at<2>() == 103.0);
	REQUIRE(mv16.at<3>() == 107.0);
}
